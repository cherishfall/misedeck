...
## Issue 9【重中之重】: 工具页深度 UX 审查（18 条，全部定稿照修）

**用户原话**: 「再一次审查下工具页面，这个页面是这一版的重中之重，要耗费大精力去优化这个页面，再看看还有没有可以优化的展示、交互和操作，因为之前adr导致你束手束脚，这次放开了手脚看下，目的是展示自然和易用，不用读文档就可以上手用了」

**审查方法**：子代理通读 ToolsPage 全部实现（1033 行 + VersionQuerySection + 双样式表 + i18n 文案 + 组件原语），按"首次使用不读文档能否上手"标准逐区审查。已排除本批次已定稿项（固定列宽、列宽拖拽、过滤/复制/排序、移除全部升级、安装=primary、刷新上移）。

**定稿（2026-09-08，用户确认全部推荐）**——ticket 切分草案：

| Ticket | 内容 |
|---|---|
| T1 文案 | F1 空状态改引导 GUI 内操作（安装表单/Registry），双语；F17 死 key `tools.installHint` 复用为 F10 ghost 链接或删除 |
| T2 表格信号语义 | F2 未激活行：版本 dim + 「未激活」小字标示（不加新徽章概念）；F6 「最新」列渲染 `当前 → 最新` 全路径（用 `→`）、删多余徽章、Tooltip 带全路径；F7 版本号颜色保持稳定（数据就是当前状态），注意力全给最新列 |
| T3 键盘 + 输入词汇 | F3 四个表单全部 `<form onSubmit>` 回车提交、切换框 Esc 还原；F4 切换框专用短占位（`22.11.0`）；F5 切换框 datalist 列该工具已装版本 + latest，查询输入 datalist 列已知工具名 |
| T4 Tooltip 守卫 + token + 条件列 | F8 Tooltip 组件内对空值/「—」不弹（一处修全站）；F14 11px/13px 硬编码字号归 token；F15 Created 列改条件列（≥1 行有值才渲染，抄 backend 列模式） |
| T5 表单与分区可理解性 | F9 链接表单：picker 降 secondary + 借 mise 原话加一行 link 释义；F10 安装表单加「浏览 Registry」ghost 链接 + 版本留空=默认/latest（占位写明）；F11 点工具名预填并执行已安装版本查询；F12 查询区命令预览实时随输入显示 |
| T6 工具栏 | F13 outdated 提示位置/跳动 + F18 刷新按钮换共享组件——并入本批次"刷新上移顶部工具栏" ticket |
