...
- Version switching picks from installed versions via a dropdown (`mise use` on selection) — never free typing, never a datalist: a version that does not exist on disk cannot be submitted (issue #132). The current version is marked (`aria-current`) and disabled, since re-selecting it would be a no-op. Installing a version that is not on disk yet belongs to the version-query sections, not the switch control.
