...
- Version switching picks from installed versions via a dropdown (`mise use` on selection) — never free typing, never a datalist: a version that does not exist on disk cannot be submitted (issue #132). The current version is marked (`aria-current`) and disabled, since re-selecting it would be a no-op. Installing a version that is not on disk yet belongs to the Add a tool section below the table, not the switch control (issue #178).
- Add tool (Tools page, issue #178, superseding #133 and #134): the region below the tools table searches the registry and, once a tool is picked, renders three version sections in a fixed order — in use (the active rows, topmost, never re-sorted; the active badge is the single active signal), installed (Use + Uninstall, the latter confirmed), and not installed (remote minus the on-disk set; Use primary — installs and activates — and Install only secondary), with a pinned `latest` row at the top of the last section annotating the concrete version it resolves to. Section order is constant: in use → installed → not installed. The version-descending sort uses a prefix-aware comparator (vendor prefixes like `graalvm-community-` plus metadata suffixes like `.LTS`/`+35`), and the filter and pagination are client-side so thousand-version lists (java) stay usable. Searching never installs: the pick only loads the version list, and every install/use action names its exact version (beta12 2-c).
