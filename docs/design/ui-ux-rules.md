...
- An action cell whose row has no reachable action renders a dim `—` — a blank action cell is a bug. The reason must be discoverable in the row itself, typically on that row's source-badge Tooltip. A row whose single action is blocked renders a disabled button + Tooltip instead; two side-by-side disabled buttons are noise, not information (beta11, issue #148).
...
- Version switching picks from installed versions via a dropdown (`mise use` on selection) — never free typing, never a datalist: a version that does not exist on disk cannot be submitted (issue #132). The current version is marked (`aria-current`) and disabled, since re-selecting it would be a no-op. Installing a version that is not on disk yet belongs to the expanded row's version center, not the switch control.
- Version center (Tools page, issue #133): clicking a tool name toggles an inline expanded row — one row expanded at a time, no caret glyph on the trigger. The center has two sub-lists. Installed versions: the active version carries the active badge as the *single* active signal — never pair it with a second one (an Active column or an "inactive" note); non-active versions offer Use and Uninstall, the active one neither. Available versions come from one cached `mise ls-remote` per (directory, tool); already-installed versions are marked and offer no action; the rest offer Use (primary — installs and activates) and Install only (secondary). Filter, version-descending sort, and pagination are client-side so thousand-version lists stay usable.
...
- No dead ends: anything the user can browse offers its natural next step. The add-tool entry is the Tools page's top registry search (`mise use`, issue #134); the Plugins page keeps only installed-plugin management (#136 removed its Registry table — browse-and-install lives in the Tools search).
