...
- Version center (Tools page, issue #133): clicking a tool name toggles an inline expanded row — one row expanded at a time, no caret glyph on the trigger, and expanding is disabled while a foreground command runs (collapsing always works). The center has two sub-lists. Installed versions: the active version carries the active badge as the *single* active signal — never pair it with a second one (an Active column or an "inactive" note); non-active versions offer Use and Uninstall, the active one neither. Available versions come from one cached `mise ls-remote` per (directory, tool); already-installed versions are marked and offer no action; the rest offer Use (primary — installs and activates) and Install only (secondary). Filter, version-descending sort, and pagination are client-side so thousand-version lists stay usable.
