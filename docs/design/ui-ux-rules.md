...
- Run-locking (issue #135): while a foreground command runs, only **command-firing controls** disable (the single-flight rule, ADR-0005, is unchanged — at most one command at a time). Browsing of already-loaded data never locks: expanding/collapsing rows, filtering, sorting, and paginating all keep working through a multi-minute install.
