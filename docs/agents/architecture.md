...
- **Directory context** (see CONTEXT.md): a single app-level state, defaulting to Global. Every runner call receives it and passes `-C` for it: Directory mode passes `-C <dir>`; Global mode passes `-C $HOME`, so mise resolves from the user's home directory — never from the process cwd (issue #179). No page hardcodes a directory.
...
- **Execution panel** (issue #15, ADR-0005) is the single path for *every* mise invocation, reads included: it shows the exact command being run and streams its logs, and `run()` returns the result so a read feeds the query cache without invoking mise twice. Reads the app issues on its own behalf (a table's initial load, a post-mutation refresh) run in the background — same runner, but they never replace the transcript the user is reading.
