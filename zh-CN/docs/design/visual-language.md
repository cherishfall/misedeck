...
- **flare** 标记*可行动的漂移*（有更新版本、警告）。本文档只管颜色语义；过时版本如何渲染（升级路径字形）由 `ui-ux-rules.md` 约束。
...
- 间距取自 `--space-*` 标尺（2 / 4 / 6 / 8 / 10 / 12 / 14 / 16 / 22 / 26px；半步索引插在基础步之间，如 `--space-1-5` = 6px）——标尺是唯一依据，底下没有算术网格。硬编码 px 的 padding/margin/gap 会被 `npm run lint:css-spacing` 拒绝。
