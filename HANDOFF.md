...
Last updated: 2026-09-01 — #38 closed; #39 is the active ticket.

**Status:** v1 product-logic implementation in progress. 5 of 11 `ready-for-agent` v1 tickets closed; #39 is the next ticket to pick up.
...
**Lint gates at the end of #38:** `npm run ci` (typecheck + `lint:i18n` + `lint:css-tokens` + build) green; `cargo check` green; i18n parity at 341 keys.

---

## Paused work

- **#39 — Execution panel on demand: hidden by default, slides in on run, absent on read-only pages** is the active ticket.
- No commit for #39 has been made yet.
...
1. **#39** Execution panel on demand: hidden by default, slides in on run, absent on read-only pages  
   → blocked by: none. Start here.
2. **#40** Command-teaching page headers + data-is-never-uppercased typography audit  
   → blocked by: #38.
3. **#41** Env page: first-class env-vars management (mise env / set / unset)  
   → blocked by: #38.
4. **#42** Preview absorbs config-file hierarchy (mise config: loaded files in precedence order)  
   → blocked by: #38.
5. **#44** Home (mise status) page in the new shell  
   → blocked by: #38.
6. **#43** Retire the Config page (redirect /config to /preview, remove residue)  
   → blocked by: #41, #42.
...
4. Run `gh issue view 39 --comments`, then implement per acceptance criteria.
...
6. Commit to `master` referencing `#39`, push with:  
   `git -c http.proxy=http://127.0.0.1:7890 push origin master`
7. Close the issue with `gh issue close 39 --comment "..."`.
8. Continue with #40–#44 in the same dependency order, one ticket per session/subagent.
